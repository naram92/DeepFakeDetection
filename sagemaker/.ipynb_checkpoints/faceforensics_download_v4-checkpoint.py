...
def download_file(url, out_file, report_progress=False):
    try:
        # Ouvrez le fichier à partir de l'URL
        with urllib.request.urlopen(url) as response:
            # Récupérez la taille du fichier
            file_size = int(response.info()['Content-Length'])

            # Utilisez tqdm pour afficher une barre de progression pendant le téléchargement
            if report_progress:
                file_data = tqdm.tqdm(response, total=file_size, unit='B', unit_scale=True, desc=out_file)
            else:
                file_data = response

            # Lisez les données téléchargées en mémoire
            file_data = response.read()

        # Créez un objet en mémoire à partir des données du fichier
        file_obj = io.BytesIO(file_data)

        # Envoyez l'objet en mémoire directement dans S3
        s3.put_object(Bucket=bucket_name, Key=out_file, Body=file_obj)
